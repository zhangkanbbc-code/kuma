...
// 镝的战前「精确档」在维护者正打的这期活动上当场变哑，而且一条报错都不会有。
//
...
//   · `ledger`  游戏报文核对（维护者核 2026-09-06）：第一方、机器记的，脚本自己读得到（`loadLedgerEventVotes`），带难度列。
...
//（未列出 ≠ 确认不掉，本项目一贯口径）。所以掉落域的第二张票只有游戏报文核对（维护者核 2026-09-06）。
 ...
 *
 * ---- 怎么逮住的（2026-08-24，维护者复查 62-5 X 点问出来的）----
 *
...
/** 一手实测那一根：游戏报文核对（维护者核 2026-09-06）与 KCNav 人肉见证，都独立于 wiki 那条转录链。 */
...
export const RESOLVED_EVENT_COMP_CONFLICTS = Object.freeze([
  {
    ...
    why:
      'KCNav 编成统计（维护者核 2026-08-24）： G 点的实测终审票（kuma 对该站零请求，' +
    ...
    evidence: [
      '一手实测：KCNav 编成统计（维护者核 2026-08-24）——G 点按丙难度过滤后三行，' +
  ...
  },
  {
    ...
    decidedAt: '2026-08-24',
    why:
      'KCNav 编成统计（维护者核 2026-08-24）： X 点的实测终审票（kuma 对该站零请求，' +
    ...
    evidence: [
      '一手实测：KCNav 编成统计（维护者核 2026-08-24）——X 点按丙难度过滤后三行，' +
...
/**
 * **KCNav 人肉见证台账**：维护者人肉浏览 KCNav（kuma 对该站零请求）逐条记下来的实测样本。
 * 认领上的编成加一票 `kcnav`——它与游戏报文核对（维护者核 2026-09-06）同属「一手实测」那一根，
 * 于是那一条从「同源转录」升到「多源一致」（wiki 一根 + 实测一根，两根独立）。
 *
 * ---- 发票的门槛：身份要被**逐格数值指纹**钉住 ----
 *
 * 见证是人眼读悬浮卡再转述，比机器记的游戏报文多两道手。所以「我在这一点看到过 N 次」
...
/**
 * 游戏报文核对（维护者核 2026-09-06）（第一方一手）。**有 IO**，与上面的纯逻辑分开。
 *
...
export const loadLedgerEventVotes = ({ dbPath = null, mapAreaId = 0 } = {}) => {
  ...
  try {
    ...
    console.warn(`[lodes]   游戏报文核对（维护者核 2026-09-06）读不到（第一方那张票缺席）：${error.message}`)
  }
