...
// kuma-cache:// URL（由主进程 kcs-resource.ts 供出），否则 undefined 走原网络请求。
const createResourceResolver = (remote) => {
  ...
  // 卡的是游戏 webview 自己的线程，kuma UI 还在动。
  const cacheDir = config.get('kuma.cache.path', remote.getGlobal('DEFAULT_CACHE_PATH'))
  return createResourceLookup(cacheDir)
}

// MAIN WORLD
// 经 contextBridge.executeInMainWorld 序列化进页面主世界。
// 必须自包含：只引用全局量与 window.kumaPreloadBridge。
function installResourceHack() {
  ...
  window.hackResource = (win = window) => {
    ...
    // 否则纹理被污染（texImage2D 直接抛错）。kuma-cache:// 响应带
    ...
    // 返回 kuma-cache:// URL（魔改或普通缓存皆可）。
    win.addEventListener(
      'error',
      (e) => {
        const el = e.target
        if (el && el.tagName === 'SCRIPT' && el.src && !el.dataset.kumaResourceRetried) {
          const hackedUrl = bridge.resolveHackedResource(el.src, true)
          if (hackedUrl) {
            const script = win.document.createElement('script')
            script.src = hackedUrl
            script.dataset.kumaResourceRetried = '1'
            if (el.parentNode) {
