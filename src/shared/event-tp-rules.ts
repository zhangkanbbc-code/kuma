...
//
// ### 游戏结算报文核对（维护者核 2026-08-27）
// 结论：使用精确值累加后取整；171.75 → 171。逐件166、逐舰167不符。
// 编成、战斗过程和结算原始记录另存维护者侧。
//
// ### 「合计」的范围：分队各自取整（上游原文）
//
...
//
// 游戏报文核对（2026-08-27）未区分整队与分队取整；分队按上述 wikiwiki 原文。
// A胜与其他难度暂无独立核对结论；上游按图给表，暂作全难度通用。
...
//
// - wikiwiki 表逐格核对；ドラム缶 一行存在来源分歧，见行注。
//
// ## 逐格核对结论（2026-08-27）
//
...
export const EVENT_TP_RULES: readonly EventTpRule[] = [
  {
    ...
    scopeNote:
      ...
      '并由 2026-08-27 对照资料实测（游戏结算 TP 171）复核',
    equips: [
      // ── 大発／特大発 系 ──
