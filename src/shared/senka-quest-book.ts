// 任务战果**入账的判据**（2026-09-01 维护者拍板，二次翻车后立的）。
//
...
// 所以口径整个换掉，三条：
//
// 1. **入账只认硬证据。** 一笔任务战果要进账，必须在游戏报文 events 里找到
...
/**
 * 从游戏报文 events 的 `post_body` 里读出任务号。
 *
...
export interface ManualQuestSenkaInput {
  ...
  at: number
  /** 游戏报文里这个任务已有的行的时间戳（手动行与观测行都要给） */
  bookedTs: readonly number[]
}

/**
 * 玩家自己补一笔任务战果（2026-09-01 维护者要的权利：季中才装上 kuma 的玩家，
 * 之前交过的任务游戏报文里不可能有证据）。形态照氪金那族的手动补记——
 ...
 * 同一个季度就会被记第二笔——维护者明确要的是「季任九月补了，坑占到该周期结束」。
 ...
 *（维护者核 2026-09-01，f3543a3 后续订正）。
 *
 * 在此之前那张单子填的是**推断**（前置满足 + 不在任务表 = 已交付）。推断在月初
 * 重置时会把没有观测计数的任务误列为已交付。
 * 入账层 f3543a3 已改用观测证据，提示层需要采用相同条件。
