...
//
// ---- 只录裁过的 ----
// 同病嫌疑还有好几件（`PENDING_EQUIP_UPGRADE_SUSPECTS`）。**不许替维护者拍板**：
// 没实测过的一律照上游显示，差异列进报告等他人肉裁，裁一件加一件，机制不变。
//
// ---- 每条都要写清证据等级 ----
// `basis` 里「游戏实测」与「表对照 + 同因推定」是两回事，不许混着写。
// 322 经游戏改修画面核对（2026-08-25）；294 依据 kcwiki 表与同因结构推定，
// 补进去的那一档**谁都没在游戏里看过**——这一点必须留在台账上，
// 免得下一个人把它当成已经验过的事实。
//
// ============================================================================
// 机制通则（2026-08-25 维护者裁决，长期判据）
// ============================================================================
// 上面那两件是一件一件裁的。裁到第三件时维护者把**机制**本身定了下来，
...
//
// 维护者拿 kcwiki 改修表（该页 2016 年后未再编辑，所以结构可信、助手名单不含 2016 年后
...
export const EQUIP_UPGRADE_CORRECTIONS: readonly EquipUpgradeCorrection[] = Object.freeze([
  {
    ...
    basis:
      '游戏改修画面实测（维护者核 2026-08-25）：最上改二特当二号舰能改 ★0 的这一件（改修资材 10 / 开发资材 8，' +
  ...
  },
  {
    ...
    basis:
      'kcwiki 改修表对照 + 322 同因结构推定，未游戏实测（尚无独立实测结论）·2026-08-25',
    improvement: [
      {
...
const TRUE_BY_RULE =
  '维护者机制通则裁决（改修支持即全档 ★0→★max，仅更新有专属二号舰）+ 322/294 同因 · 2026-08-25'

/** 通则二那一批分开写：援引的是另一条，证据来路不该混成一句话。 */
const TRUE_BY_RULE_TWO = '维护者机制通则第二条（能更新必能全档改修）· 2026-08-25'

/**
 * 适用机制通则的装备。**只对名单里的生效**，不是全库扫描——
 * 通则是维护者裁的，可名单还是得一件一件过眼睛，免得哪天上游冒出一件形状不同的
 * 被算法顺手改了。
 *
