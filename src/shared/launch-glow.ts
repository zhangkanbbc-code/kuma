...
// 舰C自己还在黑屏加载（第一次登录、或者被风控要求重新登录时更久），kuma这边的
// 面板也正在装配、缓存正在回填。这一屏把那段空白兑成一句招呼，
// 等到差不多有东西可看了再让位给点亮仪式。
//
// **屏幕上只有一块铭牌**：「欢迎返港，」加提督名，此外一样东西都没有。
// 2026-08-25 维护者裁决「大道至简，直接显示一个这个，然后淡出主界面」——上一版还摆了
...
//
//   ① 三件「真就绪」到齐
//        snapshot  快照回放到了渲染层（主进程从游戏报文回放的那份状态；提督名也是这一刻才知道）
...
export const LAUNCH_GLOW_CONFIG_KEY = 'kuma.launchGlow'
export const LAUNCH_GLOW_DEFAULT = true

/**
 * 点火节奏的三个变体。渲染层拼成 `kuma-glow-<代号>` 当 keyframes 名字用，
...
// ---- 顶栏浮层（铃/史/钥）· 打开时的入场 ----
//
// 与仪式各幕**不是一回事**，但归同一个 `kuma.launchGlow` 开关管（一个实验开关管
