...
//
// ---- 这个域为什么存在（2026-08-23 维护者拍板）----
// 2026-08-23：缺少转写时仍按已知语音槽提供播放占位，取得可靠转写后再补文字。
//
// 在这之前，文本层全空的形态台词卷是**整页空白**：没有词就没有行，没有行就没有钮，
// 于是新实装舰在kuma里等于不存在。可「这艘舰在 2 号槽会说话」这件事**与有没有文字无关**——
...
// ---- 无配音格可点重探（2026-08-23 维护者拍板）----
//
// 那时台账还有 90 天的自动复探期，它解决的是「官方后来实装了，那一格自己会回来」，
// 可它太钝：官方今天补了配音，玩家得等到第 90 天才看得见。维护者的裁定是
 ...
 * 对照资料台账里 Richelieu改 的这一条被记成「认不出」16 次，也就是说**她确实说了**，
...
// 而**那 24 条台词kuma其实有**，只是记在链上另一形态名下。这一行就是去那边的路标。
//
// ---- 为什么只指路、不下结论（要害，别写歪）----
