...
test('顶栏与锱从共用 cue 渲染同款常亮，并在 reduced-motion 下关闭光效', () => {
  ...
  assert.match(html, /\.tile\.glow-up::after, \.mod-zi \.tile\.glow-down::after \{ animation: kuma-res-glow-in 240ms ease-out both; animation-delay: calc\(-1 \* var\(--glow-elapsed, 0ms\)\); \}/)
  assert.match(html, /\.tile\.glow-down\.leaving::after \{ animation: kuma-res-glow-out 420ms ease-in forwards; animation-delay: calc\(-1 \* var\(--glow-elapsed, 0ms\)\); \}/)
  assert.match(html, /@keyframes kuma-res-glow-in \{ from \{ opacity: 0; \} to \{ opacity: 1; \} \}/)
  assert.match(html, /@keyframes kuma-res-glow-out \{ from \{ opacity: 1; \} to \{ opacity: 0; \} \}/)
