...
test('第三方零请求：会被拿去 fetch 的绝对地址，主机只能是游戏服务器或玩家自填的推送地址', () => {
  ...
  const ALLOWED_HOSTS = new Map([
    ...
    // 浏览窗的主页就是它。那不是产物自己发的请求——玩家按了顶栏「新窗」才会有
    // 一扇窗去加载它，之后去哪也全由他在地址栏里定。
    // 2026-08-30 从舰C 详情页换过来：详情页会触发 DMM 的会话动作，把主窗正在玩的
    // 那一局挤下线（玩家实测）。
    ['www.dmm.com', '浏览窗主页（玩家按「新窗」才开）· 之后去哪由玩家在地址栏定'],
    // 详情页已经不是任何默认落点，但 page-align.js 还拿它做 location.href 比较
    ['games.dmm.com', 'page-align 那处只做比较，不导航也不请求'],
